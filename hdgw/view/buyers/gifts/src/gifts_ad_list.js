    ...
    var init = function () {
        ...
        var _promotion_html = juicer([
            ...
            '<li class="for_img_item">',
                '<a href="http://item.hdgw.com/${item.product_id}.html" target="_blank" title="${item.product_name}"><img src="${item.product_img_default}" /><span class="promotion_name">${item.product_name}</span></a>',
            '</li>',
            '{@/each}'
        ].join(''));
