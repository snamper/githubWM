	...
	var init = function(){
		...
		var _domHtml = juicer([
            	'<div class="already_buy_title">',
                	'<b>已买到的商品</b>',
                    '<a href="http://order.hdgw.com/orders/myorders?SelectOrderStatus=UnPay">待付款 <span>${payCount}</span></a><a href="http://order.hdgw.com/orders/myorders?SelectOrderStatus=Sent">待确认收货<span>${receGoodsCount}</span></a><a href="http://order.hdgw.com/orders/myorders?SelectOrderStatus=Success">待评价<span>${commentCount}</span></a>',
                '</div>',
                '<div class="already_buy_con">',
                	...
                	'<div class="already_detail clearfix">',
                        ...
                        '</div>',
                        '<div class="dobuyer">',
							'<a href="http://order.hdgw.com/order/payment?orderid=${item.id}" class="buyer_btn">确认收货</a>',
                         '</div>',
                    '</div>',
					'{@/each}',
					'{@else}',
		...
		if($buy_car.length){
			$.ajax({
				ulr:"http://cart.hdgw.com/async/cart/items",
				type:"get",
				dataType:"json",
				success:function(data){
				...
				error:function(){
					var data = {"cartitem":[{"name":"圣晶 乌拉圭特级天然紫水晶手链 时尚送女友圣诞节礼物爱的守护石","price":869.00,"productlink":"http://item.hdgw.com/230169.html","imgurl":"http://img.wumeiwang.com/m00/00/17/rbaa_vkcdjmayohuaapllc2_o0q586.jpg"},{"name":"圣晶 天然水晶手链女 青金石手串珠子手链单圈 韩版时尚首饰品","price":189.00,"productlink":"http://item.hdgw.com/229936.html","imgurl":"http://img.wumeiwang.com/m00/00/0f/rbaa_fkyr4-atskyaammwvgu1qe425.jpg"},{"name":"圣晶 天然水晶手链女 青金石手串珠子手链单圈 韩版时尚首饰品","price":189.00,"productlink":"http://item.hdgw.com/229936.html","imgurl":"http://img.wumeiwang.com/m00/00/0f/rbaa_fkyr4-atskyaammwvgu1qe425.jpg"}]};
					$buy_car.empty().append(_carHtml.render(data));
				}
			});	
		}
