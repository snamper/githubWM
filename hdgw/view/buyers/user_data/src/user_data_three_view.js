define(function (require, exports, module) {
    ...
    var initTagList = function () {
        $.ajax({
            url: domains.api2 + "/system/tag/random",
            type: "get",
            dataType: "jsonp",
    ...
    var bind = function () {
            ...
            '<div class="ss_sub_remark">',
                ...
                '<ul>',
                    '{@each strong_recommend as sr}',
                        '<li><a href="' + domains.s + '/list/${sr.id}.html"  target="_blank">${sr.name}</a></li>',
                    '{@/each}',
                '</ul>',
            '</div>',
        ...
        var appendComm = function (random) {
            ...
            $.ajax({
                url: domains.s + "/search/product/by_class/show_batch",
                type: "get",
                dataType: "jsonp",
                data: {
                    category_ids: _ids,
                    ...
                    var data = [
                            {
                                id: 0,
                                product_name: "暂无数据",
                                pic: domains.a + "/hdgw/view/public/img/img404_1.png"
                            },
                            {
                                id: 0,
                                product_name: "暂无数据",
        ...
        $page.on("click", ".add_new_tag", function () {
            ...
            if (_v) {
                $.ajax({
                    url: domains.profile + "/buyer/tag/add",
                    type: "get",
                    dataType: "jsonp",
                    data: {
        ...
        $page.on("click", ".remove_tag_item", function () {
            ...
            $.ajax({
                url: domains.profile + "/buyer/tag/delete",
                type: "get",
                dataType: "jsonp",
                data: {
                    tagName: encodeURIComponent($.trim($this.attr("data_name")))
                },
                success: function (data) {
                    if (data && data.success) {
