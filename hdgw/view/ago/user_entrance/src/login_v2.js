define(function (require, exports, module) {
    ...
    var bind = function () {
        var v_name = function (_v, callback) {
            $.ajax({
                url: 'http://account.hdgw.com/accountExists',
                cache: false,
                data: {
        ...
        var v_code = function (_v, callback) {
            $.ajax({
                url: 'http://account.hdgw.com/CheckAuthCode',
                cache: false,
                data: {
                    vcode: _v
                },
                ...
                v_name(v, function (data) {
                    if (!data.response) {
                        ...
                        if (data.attachment.require_authcode && !$form.find(".v_code_txt").length) {
                            $form.find("#pw_row").after('<li class="form_row"><input type="text" class="form_txt mb10 w170 v_code_txt" placeholder="验证码" wmv="empty" wmvmsg="验证码不能为空！"><img src="http://account.hdgw.com/auth" class="mrl5 v_code_img" title="点击更换" /></li>')
                        }
                    }
                    ...
                    if (data.response) {
                        ...
                        if (lib.queryString("loginend")) {
                            try {
                                ...
                                window.location.href = (retUrl ? decodeURIComponent(retUrl) : "http://www.hdgw.com");
                            }
                        } else {
                            window.location.href = (retUrl ? decodeURIComponent(retUrl) : "http://www.hdgw.com");
                        }
        ...
        $form.on("click", ".v_code_img", function () {
            $(this).attr("src", 'http://account.hdgw.com/auth?t=' + new Date().getTime());
            return false;
        });
    };
    init();
