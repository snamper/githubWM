define(function (require, exports, module) {
    ...
    var bind = function () {
                ...
                success: function (data) {
                    if (data.response) {
                        ...
                        if (data.attachment.require_authcode && !$form.find(".v_code_txt").length) {
                            $form.find("#pw_row").after('<li class="form_row"><label class="row_key">验证码：</label><input type="text" class="form_txt w100 v_code_txt" wmv="empty" wmvmsg="验证码不能为空！" tabindex="3"><img src="' + domains.account + '/auth" class="mrl5 v_code_img" title="点击更换" style="vertical-align: middle;" /></li>')
                        }
        ...
        $form.on("click", ".go_reg", function () {
            top.location.href = domains.account + '/register';
            return false;
        });
        ...
        $form.on("click", ".v_code_img", function () {
            $(this).attr("src", domains.account + '/auth?t=' + new Date().getTime());
            return false;
        });
    };
    init();
