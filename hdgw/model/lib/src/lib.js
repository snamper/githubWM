...
define(function (require, exports, module) {
    ...
    var _islogin = function (callback, showLoginBox) {
        ...
        var _settimeout = setTimeout(function () {
            _invBox = box.invBox({
                content: '<p style="text-align: center;line-height: 70px;"><img src="http://a.myhd.wumeiwang.com/hdgw/view/public/img/loading/loading18_18_1.gif" style="vertical-align: middle;" /> 正在检测用户数据！</p>',
                btns: []
            });
        }, 1000);
        $.ajax({
    ...
    var _cookie = function (key, value) {
        ...
        //存在value，视为赋值
        if (value) {
            document.cookie = encodeURIComponent(key) + "=" + encodeURIComponent(value) + ";path=/;domain=.hdgw.com;expires=" + date.toGMTString();
    ...
    var _removeCookie = function (key) {
        ...
        document.cookie = encodeURIComponent(key) + "=x;path=/;domain=.hdgw.com ;expires=" + date.toGMTString();
    };
    ...
    var _addFavorite = function (sURL, sTitle) {
        try {
            window.external.addFavorite(sURL || window.location.href, sTitle || "互动购物");
        }
        catch (e) {
            try {
                window.sidebar.addPanel(sTitle || "互动购物", sURL || window.location.href, "");
            }
            catch (e) {
